...
export const saleFees = (province: Province, houseValue: number) => {
  let salesTax = 1;
  // Note: GST/HST applies to realtor fees across Canada, adjusted for province rates.
  // This is a simplified model. Check specific provincial rules.
  // https://wowa.ca/calculators/gst-hst-on-real-estate-commission
  switch (province) {
    case Province.AB:
      salesTax = 1.05; // GST only
      break;
    case Province.BC:
      salesTax = 1.05; // GST only (BC removed HST from services)
      break;
    case Province.ON:
    case Province.Toronto:
      salesTax = 1.13; // HST
  ...
  return sum(
    750, // legal fees (estimate)
    sum(
      Math.min(houseValue, 100000) * 0.03, // 3% on the first $100k (estimate)
      Math.max(houseValue - 100000, 0) * 0.015 // 1.5% on the rest (estimate)
    ) * 2 * salesTax // 2 agents + sales tax on commission
  );
};
